...
* **Entropy Generation**: Certain instructions generate entropy, which is added to the `SR`. Complex or resource-intensive operations produce more entropy.

* **Entropy Dissipation**: Organisms must actively reduce their entropy to survive. (Mechanisms for entropy dissipation are defined by the simulation configuration.)
...
* `IFPR %VEC_REG`, `IFPI <Vector>`, `IFPS`: If cell at `DP` + vector is passable (empty or owned by self). The vector must be a unit vector. (Cost: 1)
...
* `INMR %VEC_REG`, `INMI <Vector>`, `INMS`: If cell at `DP` + vector is **not** owned by self. The vector must be a unit vector. (Cost: 1)
* `INPR %VEC_REG`, `INPI <Vector>`, `INPS`: If cell at `DP` + vector is **not** passable (not empty and not owned by self). The vector must be a unit vector. (Cost: 1)
...
* `PEEK %DEST_REG %VEC_REG`, `PEKI %DEST_REG <Vector>`, `PEKS`: Reads and consumes molecule at `DP` + vector, then clears ownership on that cell. (Base Cost: 1)
  ...
  - STRUCTURE: If the cell is not accessible (not owned by self), pay additional `|value|` energy.
  - CODE/DATA: Pay an additional flat cost of `5` unless the cell is owned by self. Unowned and foreign-owned cells incur this `5` cost.
...
* `SEEK %VEC_REG`, `SEKI <Vector>`, `SEKS`: Moves active `DP` by vector if target cell is empty or accessible (owned by self). (Cost: 1)
...
* `FORK %DP_VEC_REG %NRG_REG %DV_VEC_REG`: Creates a child organism at `DP` + delta vector. After the child is created, all molecules owned by the parent that have a marker value equal to the parent's current `MR` are transferred to the child, and their markers are reset to 0. (Cost: 1 + energy)
* `FRKI <DP_Vec> <NRG_Lit> <DV_Vec>`, `FRKS`: Creates a child organism (immediate/stack variants). Same ownership transfer behavior as `FORK`. (Cost: 1 + energy)
* `ADPR %REG`, `ADPI <Literal>`, `ADPS`: Sets the active Data Pointer index. (Cost: 1)
* `SMR %REG`, `SMRI <Literal>`, `SMRS`: Sets the Molecule Marker Register (`MR`) to the value from the register, literal, or stack. The operand must be of type `DATA`; otherwise, the instruction fails. The value is masked to 4 bits (0-15). (Cost: 1)
