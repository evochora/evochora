...
- **STATE**: A DATA value an organism has written for its own use. It behaves like DATA in arithmetic and comparisons, but the genome hash leaves it out, so the values an organism keeps changing are separated from the genome it passes on (Section 2.3 says how a write becomes STATE).
