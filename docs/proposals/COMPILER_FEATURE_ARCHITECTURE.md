...
| D7c | (refactoring) | No feature move. **[+fix]** Replace silent fallback `return new IrLabelRef(node.toString())` in `IrGenContext.convertOperand()` with `throw new IllegalArgumentException("Unsupported operand node type: " + node.getClass().getSimpleName())`. Unhandled node types must cause an immediate, diagnosable error instead of producing invalid IR silently. | — |
| D7d | (refactoring) | No feature move. **[+DRY]** Introduce `Token.toSourceInfo()` method in `model/token/Token.java` returning `new SourceInfo(fileName(), line(), column())`. Replace all `new SourceInfo(t.fileName(), t.line(), t.column())` call sites with `t.toSourceInfo()`. Dependency direction correct: `model/token/` → `api/` (Token depends on SourceInfo, not reverse). Affected: DefineDirectiveHandler, LabelSymbolCollector, ProcedureSymbolCollector (3×), ImportSymbolCollector, RequireSymbolCollector, RegAnalysisHandler, PregAnalysisHandler. All subsequent `[+decouple]` steps (D8–D13) must use `Token.toSourceInfo()` for new extraction sites. | — |
