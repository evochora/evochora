...
| [PACKAGE_DEPENDENCY_RULES](PACKAGE_DEPENDENCY_RULES.md) | TO BE REVIEWED | Nothing stops a new import from silently adding an edge between top-level packages; an ArchUnit rule declaring the target graph, written first and red until three moves clear both cycles — a misplaced process wrapper, a broker registry stranded in `node`, and `ProgramArtifact` reaching into the ISA signature for a debug name |
