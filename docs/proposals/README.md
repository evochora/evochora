...
|---|---|---|
| [MUTATION_EVENTS](MUTATION_EVENTS.md) | TO BE REVIEWED | Mutation plugins record the cells they write as an event on the newborn, persisted once with its first recorded state like the death tick; an analytics table with one row per changed molecule makes mutation class against fate a join and exposes variation that comes from no plugin |
