...
|---|---|---|
| [LABEL_ADDRESSING](LABEL_ADDRESSING.md) | AGREED | Jumps resolve into half-built bodies, foreign code use is never heritable and a lost label is replaced by an arbitrary one; label values stable by descent, own labels first, a reach bound for foreign labels, a gradual namespace flip owned by the matching strategy, 20-bit label values shown in hexadecimal |
