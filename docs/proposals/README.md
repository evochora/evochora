...
| [PACKAGE_DEPENDENCY_RULES](PACKAGE_DEPENDENCY_RULES.md) | PROBLEM VERIFIED, APPROACHES TO BE DISCUSSED | No stated or enforced rule for dependencies between `runtime`, `compiler`, `datapipeline`, `node`, `cli`; two cycles exist today (`runtime` ↔ `compiler` via `ProgramArtifact`, `node` ↔ `datapipeline`); three enforcement approaches listed for discussion |
