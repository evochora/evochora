...
| Organism | `runtime/model/Organism.java` | `toUnitVector(int[])`: component-count check per Decision 5, `nearest`, and a **fresh copy** of the DV where `nearest` returned `null` — never the DV array itself, so parent and child cannot share one. `isUnitVector` is removed once its last caller is gone |
...
| 3 | Neighbour access, environment | the single coordinate derivation; `SEEK`; `SCAN`; the `getOwnerIdAt` fix in its own commit; ASSEMBLY_SPEC hunks | planning and execution address the same cell; a non-unit vector claims a cell in conflict resolution; `DeterministicExecutionTest` green |
...
| 5 | Fork and mask | the three fork deltas; `V2B`/`V2BS` with the split messages; `isUnitVector` removed; ASSEMBLY_SPEC hunks | forking with a mutated delta produces a child; every vector yields a mask below the axis limit; `ResumeForkNeutralityTest` green |
| 6 | Documentation and measurement | remaining ASSEMBLY_SPEC hunks (the environment interaction preamble); JMH tick benchmark | benchmark on the benchmark host with determinism check, announced with its duration |
