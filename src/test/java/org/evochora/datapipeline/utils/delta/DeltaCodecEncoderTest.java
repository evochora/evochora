...
class DeltaCodecEncoderTest {
    ...
    void constructor_invalidAccumulatedInterval_throws() {
        assertThrows(IllegalArgumentException.class, () ->
                new DeltaCodec.Encoder(RUN_ID, 1, 0, 10, 1));
    }
    
    @Test
    void constructor_invalidSnapshotInterval_throws() {
        assertThrows(IllegalArgumentException.class, () ->
                new DeltaCodec.Encoder(RUN_ID, 1, 5, 0, 1));
    }
    
    @Test
    void constructor_invalidChunkInterval_throws() {
        assertThrows(IllegalArgumentException.class, () ->
                new DeltaCodec.Encoder(RUN_ID, 1, 5, 10, 0));
    }
    
    @Test
    void getSamplesPerChunk_calculatesCorrectly() {
        DeltaCodec.Encoder encoder = new DeltaCodec.Encoder(RUN_ID, 1, 5, 20, 1);
        assertEquals(100, encoder.getSamplesPerChunk());  // 5 * 20 * 1
        
        DeltaCodec.Encoder encoder2 = new DeltaCodec.Encoder(RUN_ID, 1, 2, 10, 3);
    ...
    void captureTick_atSnapshotInterval_createsSnapshot() {
        ...
        DeltaCodec.Encoder encoder = new DeltaCodec.Encoder(RUN_ID, 1, 2, 2, 1);
    ...
    void captureTick_listsCellsInAscendingFlatIndexOrderWhateverTheLayout() {
        ...
        DeltaCodec.Encoder rowMajorEncoder = new DeltaCodec.Encoder(RUN_ID, 1, 2, 10, 1);
        DeltaCodec.Encoder tiledEncoder = new DeltaCodec.Encoder(RUN_ID, 1, 2, 10, 1);
    ...
    void captureTick_multipleChunks_workCorrectly() {
        ...
        // = 2 samples per chunk
        DeltaCodec.Encoder encoder = new DeltaCodec.Encoder(RUN_ID, 1, 1, 2, 1);
    ...
    void captureTick_withChunkIntervalGreaterThanOne_producesLargerChunks() {
        ...
        DeltaCodec.Encoder encoder = new DeltaCodec.Encoder(RUN_ID, 1, 2, 2, 2);
    ...
    void captureTick_withChunkIntervalThree_producesCorrectChunks() {
        ...
        DeltaCodec.Encoder encoder = new DeltaCodec.Encoder(RUN_ID, 1, 1, 2, 3);
