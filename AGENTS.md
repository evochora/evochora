...
- **Self-Contained Machine Code**: Execution reads nothing from the program artifact. What an instruction does follows from the cells it occupies and the organism's own state; anything a program needs at run time the compiler puts into the machine code, such as parameter passing through PUSH/POP marshalling around a CALL. A CALL created by mutation therefore behaves exactly like a compiled one. The artifact is display information for the visualizer and the data pipeline, and no runtime path may consult it.
